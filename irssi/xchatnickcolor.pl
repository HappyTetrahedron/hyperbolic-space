...
sub sig_own_public {
  ...
  $server->command('/^format own_msg %K<%w$2'.chr(3).'13'.'$[-13]0%K> |%n $1');
}
...
sub sig_public {
  ...
  $server->command('/^format pubmsg %K<%w$2'.chr(3).$color.'$[-13]0%K> |%n $1');
  $server->command('/^format pubmsg_me %K<%w$2'.chr(3).$color.'$[-13]0%K> |%Y $1%n');
